                            ...
                            <div class="dropdown-menu" data-bs-auto-close="outside">
                                <form class="p-3" id="supplierFilterForm">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="form-check"><input class="form-check-input" type="checkbox" value="Active" id="statusActive" name="supplierStatus"><label class="form-check-label" for="statusActive">Active</label></div>
                                        <div class="form-check"><input class="form-check-input" type="checkbox" value="Inactive" id="statusInactive" name="supplierStatus"><label class="form-check-label" for="statusInactive">Inactive</label></div>
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label">License Status</label>
                                        <div class="form-check"><input class="form-check-input" type="checkbox" value="Valid" id="licenseValid" name="licenseStatus"><label class="form-check-label" for="licenseValid">Valid</label></div>
                                        <div class="form-check"><input class="form-check-input" type="checkbox" value="Expiring soon" id="licenseDueSoon" name="licenseStatus"><label class="form-check-label" for="licenseDueSoon">Expiring Soon</label></div>
                                        <div class="form-check"><input class="form-check-input" type="checkbox" value="license_added" id="licenseAdded" name="licenseStatus"><label class="form-check-label" for="licenseAdded">Added</label></div>
                                        <div class="form-check"><input class="form-check-input" type="checkbox" value="license_not_added" id="licenseNotAdded" name="licenseStatus"><label class="form-check-label" for="licenseNotAdded">Not Added</label></div>
                                    </div>
                                    <hr>
                                    <div class="d-flex justify-content-between">
                                        <button type="reset" class="btn btn-outline-secondary btn-sm">Clear</button>
            ...
            @foreach($suppliers as $supplier)
             <tr data-supplier-id="supplier-{{ $supplier->id }}">
                    ...
                    <!--<div class="form-check form-switch form-switch-accept-reject mt-2"><input class="form-check-input" type="checkbox" role="switch" id="acceptReject1" data-id="{{$supplier->id}}"><label class="form-check-label small" for="acceptReject1">Rejected</label></div>-->
                     <div class="form-check form-switch form-switch-accept-reject mt-2">
                        <input class="form-check-input acceptRejectSwitch" 
                               ...
                               id="acceptReject1" 
                               data-id="{{ $supplier->id }}">
                        <label class="form-check-label small" for="acceptReject1">Accepted</label>
                    </div>
                ...
                <td>
                    ...
                    <div class="mt-2"><button class="btn btn-sm btn-primary">Add & View List</button></div>
                </td>
                <td>
  ...
  $(document).on('change', '.acceptRejectSwitch', function () {
    ...
    let supplierId = checkbox.data('id');
        ...
        if (confirm("Are you sure you want to continue this action?")) {
            ...
            $.ajax({
                url: "{{ route('accept.reject.supplier.all') }}",
                type: "POST",
                data: {
                    id: supplierId,
                    status: 1, // ✅ explicitly sending status = accepted
                    _token: "{{ csrf_token() }}"
                },
                success: function (response) {
                    if (response.success) {
                        toastr.success(response.message || "Supplier accepted successfully!");
                    } else {
                        toastr.error(response.message || "Failed to process supplier request.");
                        checkbox.prop('checked', false); // revert if not success
                    }
                    setTimeout(()=>{
                        location.reload()
                    },2000)
...
// }
    ...
    function addSupplierToTableAndCard(data) {
        ...
        newRow.innerHTML = `
            ...
            <td><span class="cell-secondary d-block">Not Set</span><div class="mt-2"><button class="btn btn-sm btn-primary">Add & View List</button></div></td>

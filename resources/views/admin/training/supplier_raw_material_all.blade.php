    ...
    <div class="card spec-container-wrapper shadow-sm">
        <header class="card-header spec-header">
            <div class="row gy-3 align-items-center">
                ...
                <div class="col-12 col-lg-5 col-xl-5" style="display:none">
            ...
            <div class="modal-body">
                ...
                <form id="renew-coa-form">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="new-batch-no" placeholder="e.g., BN-240801-A01">
                    </div>
                    <div class="mb-3">
...
<!-- Full Screen Modals -->
<div class="modal fade" id="coa-history-modal" tabindex="-1"><div class="modal-dialog modal-fullscreen"><div class="modal-content"><div class="modal-header"><h5 class="modal-title">COA History</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div><div class="modal-body"><!-- Content removed as per request --></div></div></div></div>
...
<script>
    ...
    function reloadGetData(){
        fetch("{{ route('fetch-data-frotend.allRawMaterial') }}")
            .then(res => res.json())
            .then(data => {
                // initApp(data); 
                    if (data.success) {
  ...
  function initApp(initialProductData,initialVendors, specifications,brandsData) {
      console.log("brandsData",brandsData);
        ...
        function applyFilters() {
            ...
            const { brand, allergens, storageTemp, coaStatus, coaExpiry, complianceStatus, reviewStatus, lastReview, riskLevel, allergenProfile, brandStatus, imageAttachment } = activeFilters;
            
            const filtered = dataToFilter.map(product => {
                if (!product.variants || product.variants.length === 0) {
                    const hasVariantFilters = Object.keys(activeFilters).some(k => ['brand', 'allergens', 'storageTemp', 'coaStatus', 'coaExpiry', 'complianceStatus', 'reviewStatus', 'lastReview', 'riskLevel', 'allergenProfile', 'brandStatus', 'imageAttachment'].includes(k));
        ...
        function renderTable(pageProducts, pageStartIndex) {
            ...
            pageProducts.forEach((product, groupIndex) => {
                const { productName,vendorNames, corporateName, regionalName, unitName, specificationName, approvedVendors, variants, productId, createdBy, authId,specificationView,riskLevel } = product;
                ...
                //     : `<div class="name-wrapper"><span class="detail-label">Specification: Data not added</span><div class="cell-actions">
                //     <a href="#" class="view-link edit-spec-btn" data-product-name="${productName}"  data-product-id="${productId}" data-current-spec="" ${disabledAttr}>Add</a></div></div>`;
                 const specContent = specificationName
                    ? `<div class="name-wrapper"><button class="btn btn-sm btn-outline-secondary view-spec-btn" data-spec-name="${specificationName}" data-spec-view="${specificationView}" style="flex-grow:1; justify-content: flex-start;">View Specification</button><div class="cell-actions"><a href="#" class="view-link edit-spec-btn" data-product-name="${productName}" data-product-id="${productId}" data-current-spec="${specificationName}" ${disabledAttr}>Edit</a></div></div>`
                    : `<div class="name-wrapper"><span class="detail-label">Specification: Data not added</span><div class="cell-actions">
                    </div></div>`;
                            ...
                            <div class="uploaded-by-info mt-2">Uploaded by: <strong>${product.uploadedBy || 'N/A'}</strong></div>
                                ...
                                <div class="action-toggle-container">
                                    ...
                                    <label class="action-toggle">
                                        <input type="checkbox" class="status-toggle" data-id="${productId}">
                    ...
                    <div class="detail-list">
                        <div class="detail-item">
                            ...
                            <div class="cell-actions">
                                 <a href="#" class="view-link view-latest-coa-btn" data-variant-id="${variant.id}" title="View Latest COA PDF">View PDF</a>
    ...
    myModal.show();
        ...
        function openCoaHistoryModal(product, variant) { coaHistoryModal.show(); }
        ...
        function updateFilterIcons() {
            document.querySelectorAll('.filter-icon').forEach(icon => {
                ...
                const relevantKeys = {
                    ...
                    labelingSpecs: ['allergens', 'storageTemp'],

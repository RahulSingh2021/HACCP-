    ...
    <tr>
        ...
        <tbody>
            <tr class="sec-box-table">
                ...
                <td>Location: 
                @if(!empty($facility_equipment->location_id))
            ...
            <tr class="sec-box-table">
                <td class="set-size1">Frequency: {{ $facility_equipment->c_frequency ?? ''}}</td>
 ...
 @if($cleaning_schedules && count($cleaning_schedules) > 0)
    @forelse($cleaning_schedules as $schedule)
        ...
        <tr class="fst-table-td">
            <td>Scheduled At: {{ $schedule->cleaning_task_start_date ?? 'N/A' }}</td>
            <td>Completed On: {{ $schedule->cleaning_task_end_date ?? 'N/A' }}</td>
            <td>Completed At: {{ $schedule->cleaning_task_end_time ? \Carbon\Carbon::createFromFormat('H:i:s', $schedule->cleaning_task_end_time)->format('h:i A') : 'N/A' }}</td>
            <td>Completed By: {{ $completedByName ?? 'N/A' }}</td>
            <td>
                @if(\Carbon\Carbon::parse($schedule->cleaning_task_start_date)->isToday())

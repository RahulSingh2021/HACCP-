        ...
        @endforeach
         @foreach($facility_equipment as $facility_equipments)
            ...
            function setupMultiSelectComponent({ containerId, config, dataProvider, selectedItems, onSelectionChange, displayFormatter }) {
                ...
                //                 </span>`;
                //              pillsContainer.innerHTML = placeholderHtml;
                //         } else {
                //             const pillsHtml = selectedItems.map(item => `<div class="pill">${item} <span class="deselect-pill" data-item="${item}">×</span></div>`).join('');
                //             const addButtonHtml = `<button type="button" class="pill-add-button" aria-label="Add another item"><svg viewBox="0 0 16 16"><path d="M8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4z"/></svg></button>`;
                //             pillsContainer.innerHTML = pillsHtml + addButtonHtml;
                //         }
                //     } else {
                //          pillsContainer.innerHTML = selectedItems.length === 0 
                //             ? `<span class="pills-placeholder">${config.svg}<span class="placeholder-text">${config.label}</span></span>`
                //             : selectedItems.map(item => `<div class="pill">${item} <span class="deselect-pill" data-item="${item}">×</span></div>`).join('');
                //     }
                // };

                // const renderDropdownList = () => {
                //     const s = searchInput.value.toLowerCase();
                //     const filteredItems = dataProvider().filter(item => formatter(item).toLowerCase().includes(s));
                    
                //     const addNewTriggerHtml = `
                //         <div class="multi-select-item add-new-trigger">
                //           <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="16"></line><line x1="8" y1="12" x2="16" y2="12"></line></svg>
                //           <label>Add New</label>
                //         </div>`;
                        
                //     const itemsHtml = filteredItems.map(item => {
                //             const formattedItem = formatter(item);
                //             const isSelected = selectedItems.includes(formattedItem);
                //             const id = `${containerId}-${formattedItem.replace(/[^a-zA-Z0-9]/g, '-')}`;
                //             return `<div class="multi-select-item ${isSelected ? 'selected' : ''}">
                //                 <input type="checkbox" id="${id}" data-item="${formattedItem}" ${isSelected ? 'checked' : ''}>
                //                 <label for="${id}" >${formattedItem}</label>
                //             </div>`;
                //         }).join('');

                //     listContainer.innerHTML = addNewTriggerHtml + (itemsHtml || `<div style="padding: 10px; color: var(--details-gray-600); text-align: center;">No results</div>`);
                // };

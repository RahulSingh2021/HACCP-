...
@php $i=1; @endphp
                                                    ...
                                                    @php $userss = DB::table('users')->where('id', $lists->unit_id)->first(); @endphp
                                                  <tr style="text-align: center;" class="{{ $lists->rowClass }}">
                                                    <td style="background: #ccc;width: 64px;" class="{{ $lists->rowClass }}">{{$i}}</td>
        <th style="background: #ccc;text-align: center;padding: 17px;width: 330px;" class="{{ $lists->rowClass }}">
            

                                                                                                
    <strong
    class="empName">{{ $userss->Company_address ?? '' }} </strong>                                                                              
    <span
    ...
    title="Date of joining">{{ $userss->mobile_number ?? '' }}</span>
        ...
        <td class="{{ $lists->rowClass }}">{{$lists->cat_type ?? ''}} </td>
        ...
        <td class="{{ $lists->rowClass }}">
                    ...
                    @if($user_type=="Corporate")
                            ...
                            <a  href="{{route('unitHistoryHra',[$id,'Corporate'])}}?documentStatus={{ $lists->rowClass }}">Update Information</a>
                            <br>
                            <a  href="{{route('allUnitHistory',[$id,'Corporate'])}}?document_type={{$documentStatus}}" >History</a>
                            
                            @else
                            <a  href="{{route('unitHistory',[$id,'Corporate'])}}?documentStatus={{ $lists->rowClass }}" >Update Information</a>
                            <br>
                            <a  href="{{route('allUnitHistory',[$id,'Corporate'])}}" >History</a>
                            @endif
            ...
            @endif
                ...
                @if($user_type=="regional")
                             ...
                             @if(!empty($documentStatus))
                                                    ...
                                                    <a  href="{{route('unitHistoryHra',[$id,'regional'])}}?documentStatus={{ $lists->rowClass }}">Update Information</a>
                                                                <br>
                            <a  href="{{route('allUnitHistory',[$id,'regional'])}}?document_type={{$documentStatus}}" >History</a>
                                                    
                                                    @else
                                                   <a  href="{{route('unitHistory',[$id,'regional'])}}?documentStatus={{ $lists->rowClass }}" >Update Information</a>
                                                                                 <br>
            <a  href="{{route('allUnitHistory',[$id,'regional'])}}" >History</a>
                                                    @endif
            ...
            @endif
                ...
                @if($user_type=="Unit")
                  ...
                  @if(!empty($documentStatus))
                                                    ...
                                                    <a  href="{{route('unitHistoryHra',[$id,'unit'])}}?documentStatus={{ $lists->rowClass }}">Update Information</a>
                                                                <br>
                            <a  href="{{route('allUnitHistory',[$id,'unit'])}}?document_type={{$documentStatus}}" >History</a>
                                                    
                                                    @else
                                                   <a  href="{{route('unitHistory',[$id,'unit'])}}?documentStatus={{ $lists->rowClass }}" >Update Information</a>
                                                                          <br>
            <a  href="{{route('allUnitHistory',[$lists->unit_id,'unit'])}}" >History</a>
                                                    @endif
                                                    
                                                    
                            <!--<a  href="{{route('unitHistory',[$lists->unit_id,'unit'])}}?documentStatus={{ $lists->rowClass }}" >Update Information</a>-->

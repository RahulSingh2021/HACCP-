                             ...
                             <tr>
                                ...
                                <th>
                                    ...
                                    <div class="dropdown-menu p-3" onclick="event.stopPropagation();">
                                        ...
                                        <div class="mb-2"><label class="form-label small">Temperature Range (°C)</label><div class="d-flex gap-2"><input type="number" id="filterTempFrom" class="form-control form-control-sm" placeholder="From"><input type="number" id="filterTempTo" class="form-control form-control-sm" placeholder="To"></div></div>
                                        <hr><div class="d-flex gap-2"><button class="btn btn-secondary btn-sm flex-grow-1 clear-filter-btn">Clear</button><button class="btn btn-primary btn-sm flex-grow-1 apply-filter-btn">Apply</button></div>
                                    </div>
                                </th>
                                <th>
...
//             vendorProductDatabase !== null  // if vendor selected → use vendor DB
//                 ? vendorProductDatabase
//                 : data.productDatabase;      // else → use main DB
      ...
      function initApps(productDetails, vendorsData, dataMain){
        ...
        function createTableRowElement(data) {
           ...
           const verifiedByName = data.verified ? data.verified_com : 'Jane Smith';
            ...
            //     const tempImgSrc = data.tempImageSrc || `https://placehold.co/80x30/dbeafe/2563eb?text=${data.temp}°C`;
            //     return `<div>
       ...
       <div class="form-check">
        ...
        function getProductRowTemplate(index) {
            return `
                <tr class="product-entry-row">
                   ...
                   <td><select class="form-select form-select-sm product-name" required placeholder="Select product..."></select><select class="form-select form-select-sm product-brand mt-1" required placeholder="Select brand..."></select></td>
                    ...
                    <td>
                        <div class="d-flex align-items-center">
                            <div class="input-group input-group-sm">
                                 <input type="number" step="0.1" class="form-control product-temp" required placeholder="°C">
        ...
        function updateDiscrepancyRowState(row) {
            ...
            if (selectedOption && selectedOption.dataset.tempType !== 'ambient' && !tempInput.disabled) {
                ...
                if (!isNaN(currentTemp)) {
                    let err = '';
                    if ('min' in selectedOption.dataset && 'max' in selectedOption.dataset && (currentTemp < minTemp || currentTemp > maxTemp)) err = `Must be ${minTemp}°C - ${maxTemp}°C`;
                    else if ('max' in selectedOption.dataset && currentTemp > maxTemp) err = `Must be below ${maxTemp}°C`;
        ...
        function generatePdfAsImage() {
            ...
            displayedRecords.forEach((rec, index) => {
                ...
                doc.text(rec.temp !== 'N/A' ? `${rec.temp}°C` : 'N/A', colX[3], currentY);
                if(rec.temp !== 'N/A') {
                    doc.setDrawColor('#dee2e6');

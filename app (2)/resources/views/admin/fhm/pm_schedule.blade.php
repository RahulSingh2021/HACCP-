...
<div class="farst-box-full">
 ...
 <div class="col-2">
            <select class="form-select select-search" name="department" onchange="this.form.submit()">
                ...
                <option value="{{$department->id ?? ''}}" {{ ( $department->id == @$_GET['department']) ? 'selected' : '' }}>{{$department->name}}</option>
        ...
        <div class="col-md-4">
            ...
            <!--<a class="btn btn-outline-dark px-3" onclick="createPDF()">Export to PDF</a>-->
                        <a href="{{route('fhm-pm-export')}}" class="btn btn-primary px-3" download> Export</a>
...
<div style="margin-top:10px">
     <table  class="table table-bordered" style="border: 2px solid #fff;">
            ...
            <th style="width: 50%;">PM Check list
                            ...
                            @foreach($cleaning_schedule as $schedule)
                                ...
                                @php
                                    $facility_equipment = DB::table('facility_equipment')->where('id', $schedule->facility_equipment_id ?? '')->first();
                                     ...
                                     $nextDueDate = DB::table('pm_schedules')
    ->where('facility_equipment_id', $schedule->facility_equipment_id) // Same equipment ka record filter
    ->where('pm_task_start_date', '>', function ($query) use ($schedule) {
        $query->select('pm_task_start_date')
            ->from('pm_schedules')
            ->where('id', $schedule->id);
    })
    ->value('pm_task_start_date');
                                    
                                    $responsibilityName = DB::table('authority')->where('id', $facility_equipment->responsibility_id ?? '')->value('name');
                                @endphp
       <tr style="background: #cdd2d8;">
        <td>  <div class="">
                                        <h6>Scheduled  Month:{{ \Carbon\Carbon::parse($schedule->pm_task_start_date)->format('F Y') ?? '' }}
</h6>
                          
                                        
                                        <h6>Next Due Month:<br/> {{ \Carbon\Carbon::parse($nextDueDate)->format('F Y') ?? '' }} </h6>
    ...
    $monthName = $facility_equipment->p_frequency ?? '' ? ($months[$facility_equipment->p_frequency] ?? '') : '';
@endphp

@if(!empty($equpiments3) && !empty($monthName))
    <h6>Frequency: {{$equpiments3}} Months ({{$monthName}})</h6>
@endif
 ...
 <a  href="{{ route('Calibration_history', $facility_equipment->id ?? '') }}" >
   View History
        ...
        <td style="padding: 0;">                   @php $AddChecklist = DB::table('template_equpiments')->where('equpiments',$schedule->facility_equipment_id)->first(); @endphp
                ...
                <br>
                @php
                $questionlist = DB::table('template_question')->where('type', "2")->where('template_id', $AddChecklist->template_id)->get();
...
@foreach($questionlist as $questionlists)
    @php
        ...
        $colorIndex++;
    @endphp
    <p style="background-color: {{ $color }};margin: 0px !important "><span style="margin: 8px !important;font-size: 14px !important;display: inline-block;line-height: 24px;">{{$questionlists->question ?? ''}}</span>:    <span>Yes</span></p>
                        ...
                        @if($cleaning_schedule && count($cleaning_schedule) > 0)
                            @foreach($cleaning_schedule as $schedule)
                                ...
                                <div class="box-top box-border-1">
                                    <div class="">
                                        <h6>PM Scheduled Date: {{ $schedule->pm_task_start_date }}</h6>
                                        <h6>{{ \Carbon\Carbon::parse($schedule->pm_task_start_date)->format('l') }}</h6>
                                    </div>
                                    <div class="">
                                        <h6>Equipment Name: {{ $facility_equipment->name ?? ''}}</h6>
                                        ...
                                        <h6>Equipment ID: {{ $facility_equipment->equipment_id ?? ''}}</h6>
                                        <h6>Status: {{ $schedule->status ?? 'Active/Inactive' }}</h6>
                                        <!--<h6>{{$schedule->facility_equipment_id}}</h6>-->
                                        <a href="{{ route('facility_hygiene_pm_schedule_history',$schedule->facility_equipment_id) }}">View History</a>
                                    </div>
                                    <div class="">
                                     
                                    <h6>Corporate: {{ DB::table('users')->where('id',$schedule->created_by)->where('is_role', 2)->value('company_name') ?? 'N/A' }}</h6>

                                    @php
                                        $firstreg = DB::table('users')->where('created_by',$schedule->created_by)->where('is_role', 1)->first();
                                    @endphp
                                    <h6>Regional: {{ $firstreg->company_name ?? 'N/A' }}</h6>
                                    
                                    @php
                                        $firstunit = DB::table('users')->where('created_by1', $firstreg->id ?? null)->where('is_role', 3)->first();
                                    @endphp
                                    <h6>Unit: {{ $firstunit->company_name ?? 'N/A' }}</h6>
                                    ...
                                    <div class="">
                                        <h6>Frequency in days: {{ $facility_equipment->p_frequency ?? ''}}</h6>
                                    </div>
                                    <div class="">
                                        @if(\Carbon\Carbon::parse($schedule->pm_task_start_date)->lt(\Carbon\Carbon::now()->subDays(2)) && empty($schedule->cleaning_task_end_date))
                                            <h6>Missed</h6>
                                        @elseif(!empty($schedule->pm_task_end_date))
                                            <h6>Attended</h6>
                                        @elseif(\Carbon\Carbon::parse($schedule->pm_task_start_date)->gt(\Carbon\Carbon::now()->addDays(3)))
                                            <h6>Upcoming</h6>
                                        @else
                                       <h6> Active</h6>
                                        @endif
